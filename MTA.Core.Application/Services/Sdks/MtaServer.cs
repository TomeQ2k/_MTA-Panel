...
namespace MTA.Core.Application.Services.Sdks
{
    /// <summary>
    ...
    public class MtaServer
    {
        ...
        public MtaServer(string host, int port)
        {
            _Host = host;
            _Port = port;
        }
        ...
        public MtaServer(string host, int port, string username, string password)
        {
            _Host = host;
            _Port = port;
            _Username = username;
        ...
        /// <returns></returns>
        public string CallFunction(string resourceName, string functionName, MtaLuaArgs args)
        {
            string requestContent = args.ConvertToJSONString();
            try
            {
                string requrl = "http://" + _Host + ":" + _Port.ToString() + "/" + resourceName + "/call/" +
                                functionName;
            ...
            catch (WebException ex)
            {
                ...
                if (ex.Message == str)
                {
                    ...
                    else
                    {
                        ...
                        throw new MTARequestException(ex.Message, ex.InnerException, WebExceptionStatus.ConnectFailure,
                            ex.Response);
                    }
                }
                else
        ...
        private string DoPOSTRequest(string url, string json)
        {
            ...
            {
                if (_ReconnectOnFailure)
                    throw new WebException(ex.Message, ex.InnerException, WebExceptionStatus.ConnectFailure,
                        ex.Response);
                else
                {
                    string msg = "";
                    ...
                    throw new MTARequestException("Access denied. " + msg, ex.InnerException,
                        WebExceptionStatus.ConnectFailure, ex.Response);
                }
            }
        }
    }
    ...
    public class MTARequestException : ApplicationException
    {
        ...
        public MTARequestException(string message, Exception innerException, WebExceptionStatus status,
